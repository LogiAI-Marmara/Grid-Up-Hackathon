...
svg.add('<rect x="722" y="765" width="410" height="92" fill="#fff" opacity="0.92" stroke="#2e7d32" stroke-width="1.5"/>')   # 2. satır 360 px kutuya sığmıyordu
svg.text(735, 790, 'MODÜL / kapak iç yüzünde', 18, '#1b5e20', weight='700')
svg.text(735, 814, ('kutu 120 × 80, X 590–710 / IR merkezi (%g, %g) mm' % (IRX, 1500-IRY)).replace('.', ','), 15, GREEN)
svg.text(735, 836, 'bakış: sayfaya dik (→ yan kesit) / klemense DİK', 15, GREEN)
svg.text(735, 852, 'kesikli çizgi = ön düzlemde, kapağa bağlı (model: GridUp-Pano.f3d)', 13, '#558b2f', style='italic')
...
comment = """    Modül yerleşimi ve termal görüş hattı / TEDAŞ-MLZ/2003-06.B, EK-II/14 üzerine.
    ...
    Model koordinatı: X sağ, Y yukarı, Z öne (kapak iç yüzü Z = 0, montaj plakası ön yüzü Z = −430, NH ön yüzü Z = −208,5).
